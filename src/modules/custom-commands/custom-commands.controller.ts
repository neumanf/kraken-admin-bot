...
export class CustomCommandsController {
    ...
    async handle(ctx: ExtendedContext): Promise<Composer<Context>> {
        ...
        const customCommands = await this.customCommandsService.findAll(chatId);
                    ...
                    try {
                        if (customCommands[i].name === ctx.message?.text?.slice(1)) {
                            if (customCommands[i].counter >= 0) {
                                ...
                                await this.customCommandsService.updateCounter(customCommands[i].name, customCommands[i].counter);
                            } else {
